// Lead Contractor Progress Dashboard
// Tracks project progress with StartD8 SDK Lead Contractor integration.
local config = (import '../../config.libsonnet')._config;
local dashboards = import '../../lib/dashboards.libsonnet';
local panels = import '../../lib/panels.libsonnet';

local tempoDs = { type: 'tempo', uid: 'tempo' };
local mimirDs = { type: 'prometheus', uid: 'mimir' };

local withGrid(panel, h, w, x, y) = panel + { gridPos: { h: h, w: w, x: x, y: y } };

local traceqlTarget(query, limit=1000) = {
  datasource: tempoDs,
  limit: limit,
  query: query,
  queryType: 'traceql',
  refId: 'A',
};

local baseDashboard = dashboards.dashboard(
  title='[BEAVER] Lead Contractor Progress',
  uid='beaver-lead-contractor',
  description='Track project progress with StartD8 SDK Lead Contractor integration - task status and execution traces from Tempo',
  tags=['startd8', 'beaver', 'project-tracking'],
);

baseDashboard {
  annotations: { list: [] },
  time: { from: 'now-7d', to: 'now' },
  templating: {
    list: [
      {
        name: 'project',
        label: 'Project',
        type: 'custom',
        current: { text: 'beaver-lead-contractor', value: 'beaver-lead-contractor' },
        options: [
          { text: 'beaver-lead-contractor', value: 'beaver-lead-contractor', selected: true },
          { text: 'ajidamoo-squirrel', value: 'ajidamoo-squirrel' },
          { text: 'contextcore', value: 'contextcore' },
          { text: 'contextcore-tui-fixes', value: 'contextcore-tui-fixes' },
          { text: 'asabikeshiinh-localization', value: 'asabikeshiinh-localization' },
          { text: 'startd8-sdk', value: 'startd8-sdk' },
        ],
        query: 'beaver-lead-contractor,ajidamoo-squirrel,contextcore,contextcore-tui-fixes,asabikeshiinh-localization,startd8-sdk',
      },
    ],
  },
  timepicker: {},
  timezone: '',
  weekStart: '',
  panels: [
    // === Row: Task Status Overview ===
    withGrid(panels.row('Task Status Overview'), h=1, w=24, x=0, y=0) + { id: 100 },

    // Completed Tasks
    withGrid({
      id: 30,
      title: 'Completed Tasks',
      type: 'stat',
      datasource: tempoDs,
      fieldConfig: {
        defaults: {
          thresholds: {
            mode: 'absolute',
            steps: [{ color: 'green', value: null }],
          },
        },
      },
      options: {
        colorMode: 'value',
        graphMode: 'area',
        textMode: 'auto',
        reduceOptions: { calcs: ['count'], fields: '', values: false },
      },
      targets: [traceqlTarget('{resource.project.id = "$project" && span.task.status = "done"}')],
    }, h=4, w=6, x=0, y=1),

    // In Progress Tasks
    withGrid({
      id: 31,
      title: 'In Progress Tasks',
      type: 'stat',
      datasource: tempoDs,
      fieldConfig: {
        defaults: {
          thresholds: {
            mode: 'absolute',
            steps: [{ color: 'blue', value: null }],
          },
        },
      },
      options: {
        colorMode: 'value',
        graphMode: 'none',
        textMode: 'auto',
        reduceOptions: { calcs: ['count'], fields: '', values: false },
      },
      targets: [traceqlTarget('{resource.project.id = "$project" && span.task.status = "in_progress"}')],
    }, h=4, w=6, x=6, y=1),

    // Avg Task Duration
    withGrid({
      id: 32,
      title: 'Avg Task Duration',
      type: 'stat',
      datasource: tempoDs,
      fieldConfig: {
        defaults: {
          unit: 'ms',
          thresholds: {
            mode: 'absolute',
            steps: [
              { color: 'green', value: null },
              { color: 'yellow', value: 60000 },
              { color: 'red', value: 300000 },
            ],
          },
        },
      },
      options: {
        colorMode: 'value',
        graphMode: 'none',
        textMode: 'auto',
        reduceOptions: { calcs: ['mean'], fields: '/.*duration.*/', values: false },
      },
      targets: [traceqlTarget('{resource.project.id = "$project"} | select(duration)', limit=100)],
    }, h=4, w=6, x=12, y=1),

    // Total Task Spans
    withGrid({
      id: 33,
      title: 'Total Task Spans',
      type: 'stat',
      datasource: tempoDs,
      fieldConfig: {
        defaults: {
          thresholds: {
            mode: 'absolute',
            steps: [{ color: 'purple', value: null }],
          },
        },
      },
      options: {
        colorMode: 'value',
        graphMode: 'area',
        textMode: 'auto',
        reduceOptions: { calcs: ['count'], fields: '', values: false },
      },
      targets: [traceqlTarget('{resource.project.id = "$project"}')],
    }, h=4, w=6, x=18, y=1),

    // === Row: Task Execution History ===
    withGrid(panels.row('Task Execution History'), h=1, w=24, x=0, y=5) + { id: 101 },

    // Task Execution History table
    withGrid({
      id: 11,
      title: 'Task Execution History',
      type: 'table',
      datasource: tempoDs,
      fieldConfig: {
        defaults: {
          custom: {
            align: 'auto',
            cellOptions: { type: 'auto' },
          },
        },
        overrides: [
          {
            matcher: { id: 'byName', options: 'task.status' },
            properties: [
              {
                id: 'custom.cellOptions',
                value: { type: 'color-background', mode: 'basic' },
              },
              {
                id: 'mappings',
                value: [
                  { type: 'value', options: { pending: { color: 'yellow', text: 'PENDING' } } },
                  { type: 'value', options: { in_progress: { color: 'blue', text: 'IN PROGRESS' } } },
                  { type: 'value', options: { done: { color: 'green', text: 'DONE' } } },
                  { type: 'value', options: { blocked: { color: 'red', text: 'BLOCKED' } } },
                ],
              },
            ],
          },
          {
            matcher: { id: 'byName', options: 'Duration' },
            properties: [{ id: 'unit', value: 'ms' }],
          },
          {
            matcher: { id: 'byName', options: 'task.percent_complete' },
            properties: [
              { id: 'unit', value: 'percent' },
              { id: 'custom.cellOptions', value: { type: 'gauge', mode: 'basic' } },
            ],
          },
        ],
      },
      options: {
        cellHeight: 'sm',
        showHeader: true,
      },
      targets: [traceqlTarget(
        '{resource.project.id = "$project"} | select(span.task.id, span.task.title, span.task.status, span.task.percent_complete, duration)',
        limit=50,
      )],
    }, h=12, w=24, x=0, y=6),

    // === Row: Task Duration Analysis ===
    withGrid(panels.row('Task Duration Analysis'), h=1, w=24, x=0, y=18) + { id: 102 },

    // Task Lead Time Distribution
    withGrid({
      id: 12,
      title: 'Task Lead Time Distribution',
      type: 'histogram',
      datasource: tempoDs,
      fieldConfig: {
        defaults: {
          unit: 'ms',
          custom: {
            fillOpacity: 80,
            gradientMode: 'scheme',
          },
        },
      },
      options: {
        bucketOffset: 0,
        legend: {
          displayMode: 'list',
          placement: 'bottom',
          showLegend: true,
        },
      },
      targets: [traceqlTarget('{resource.project.id = "$project" && span.task.status = "done"} | select(duration)')],
    }, h=8, w=12, x=0, y=19),

    // Tasks by Status (piechart)
    withGrid({
      id: 13,
      title: 'Tasks by Status',
      type: 'piechart',
      datasource: tempoDs,
      fieldConfig: {
        defaults: {
          custom: {
            hideFrom: { legend: false, tooltip: false, viz: false },
          },
        },
      },
      options: {
        legend: {
          displayMode: 'table',
          placement: 'right',
          showLegend: true,
          values: ['value', 'percent'],
        },
        pieType: 'donut',
        reduceOptions: { calcs: ['count'], fields: '', values: false },
        tooltip: { mode: 'single', sort: 'none' },
      },
      targets: [traceqlTarget('{resource.project.id = "$project"} | select(span.task.status)')],
    }, h=8, w=12, x=12, y=19),

    // === Row: Workflow Runs ===
    withGrid(panels.row('Workflow Runs (from Tempo)'), h=1, w=24, x=0, y=27) + { id: 103 },

    // Workflow Executions table
    withGrid({
      id: 40,
      title: 'Workflow Executions',
      type: 'table',
      datasource: tempoDs,
      fieldConfig: {
        defaults: {
          custom: {
            align: 'auto',
            cellOptions: { type: 'auto' },
          },
        },
        overrides: [
          {
            matcher: { id: 'byName', options: 'workflow.status' },
            properties: [
              {
                id: 'custom.cellOptions',
                value: { type: 'color-background', mode: 'basic' },
              },
              {
                id: 'mappings',
                value: [
                  { type: 'value', options: { running: { color: 'blue', text: 'RUNNING' } } },
                  { type: 'value', options: { completed: { color: 'green', text: 'COMPLETED' } } },
                  { type: 'value', options: { failed: { color: 'red', text: 'FAILED' } } },
                ],
              },
            ],
          },
          {
            matcher: { id: 'byName', options: 'Duration' },
            properties: [{ id: 'unit', value: 'ms' }],
          },
        ],
      },
      options: {
        cellHeight: 'sm',
        showHeader: true,
      },
      targets: [traceqlTarget(
        '{resource.project.id = "$project" && name =~ "workflow.*"} | select(span.workflow.id, span.workflow.status, span.workflow.type, duration)',
        limit=50,
      )],
    }, h=8, w=24, x=0, y=28),

    // === Row: StartD8 Agent Metrics (collapsed) ===
    withGrid({
      type: 'row',
      title: 'StartD8 Agent Metrics (if available)',
      collapsed: true,
      panels: [
        // Total Cost
        withGrid({
          id: 1,
          title: 'Total Cost (USD)',
          type: 'stat',
          datasource: mimirDs,
          fieldConfig: {
            defaults: {
              unit: 'currencyUSD',
              decimals: 4,
              noValue: 'No data yet',
              thresholds: {
                mode: 'absolute',
                steps: [
                  { color: 'green', value: null },
                  { color: 'yellow', value: 1 },
                  { color: 'red', value: 5 },
                ],
              },
            },
          },
          options: {
            colorMode: 'value',
            graphMode: 'area',
            textMode: 'auto',
            reduceOptions: { calcs: ['lastNotNull'], fields: '', values: false },
          },
          targets: [{
            datasource: mimirDs,
            expr: 'sum(startd8_cost_total{project="$project"})',
            refId: 'A',
          }],
        }, h=4, w=6, x=0, y=37),

        // Total Tokens
        withGrid({
          id: 2,
          title: 'Total Tokens',
          type: 'stat',
          datasource: mimirDs,
          fieldConfig: {
            defaults: {
              unit: 'short',
              noValue: 'No data yet',
              thresholds: {
                mode: 'absolute',
                steps: [{ color: 'blue', value: null }],
              },
            },
          },
          options: {
            colorMode: 'value',
            graphMode: 'area',
            textMode: 'auto',
            reduceOptions: { calcs: ['lastNotNull'], fields: '', values: false },
          },
          targets: [{
            datasource: mimirDs,
            expr: 'sum(startd8_tokens_total{project="$project"})',
            refId: 'A',
          }],
        }, h=4, w=6, x=6, y=37),

        // Cost by Agent/Model
        withGrid({
          id: 7,
          title: 'Cost by Agent/Model',
          type: 'timeseries',
          datasource: mimirDs,
          fieldConfig: {
            defaults: {
              unit: 'currencyUSD',
              noValue: 'No data yet',
              custom: {
                drawStyle: 'line',
                lineInterpolation: 'smooth',
                fillOpacity: 20,
              },
            },
          },
          targets: [{
            datasource: mimirDs,
            expr: 'sum by (agent, model) (rate(startd8_cost_total{project="$project"}[5m]))',
            legendFormat: '{{agent}} ({{model}})',
            refId: 'A',
          }],
        }, h=8, w=12, x=0, y=41),

        // Token Usage
        withGrid({
          id: 8,
          title: 'Token Usage',
          type: 'timeseries',
          datasource: mimirDs,
          fieldConfig: {
            defaults: {
              unit: 'short',
              noValue: 'No data yet',
              custom: {
                drawStyle: 'line',
                fillOpacity: 20,
                stacking: { mode: 'normal' },
              },
            },
          },
          targets: [{
            datasource: mimirDs,
            expr: 'sum by (type) (rate(startd8_tokens_total{project="$project"}[5m]))',
            legendFormat: '{{type}}',
            refId: 'A',
          }],
        }, h=8, w=12, x=12, y=41),
      ],
    }, h=1, w=24, x=0, y=36) + { id: 104 },
  ],
}
