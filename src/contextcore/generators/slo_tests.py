...
---'''
    ...
    def generate(self, project_id: str, spec: Dict[str, Any], test_types: Optional[List[TestType]] = None) -> List[GeneratedTest]:
        """
        ...
        Args:
            ...
            test_types: List of test types to generate (None = all)
    ...
    def _generate_chaos_tests(self, project_id: str, requirements: Dict[str, Any],
        ...
        for target in targets:
            ...
            base_name = f"{project_id}-{app_name}"

            # PodChaos for all criticality levels
            pod_content = (self.pod_chaos_template
                          .replace("{{name}}", base_name)
                          .replace("{{project_id}}", project_id)
                          .replace("{{namespace}}", namespace)
                          .replace("{{app}}", app_name))

            chaos_tests.append(GeneratedTest(
                name=f"{base_name}_pod_chaos",
                test_type=TestType.CHAOS,
                derived_from=f"availability={requirements.get('availability', '99.9%')}",
                content=pod_content,
                file_extension=".yaml"
            ))

            # NetworkChaos for critical and high criticality
            if criticality in ("critical", "high"):
                network_content = (self.network_chaos_template
                                  .replace("{{name}}", base_name)
                                  .replace("{{project_id}}", project_id)
                                  .replace("{{namespace}}", namespace)
                                  .replace("{{app}}", app_name))
                ...
                chaos_tests.append(GeneratedTest(
                    name=f"{base_name}_network_chaos",
                    test_type=TestType.CHAOS,
                    derived_from=f"availability={requirements.get('availability', '99.9%')}, criticality={criticality}",
                    content=network_content,
                    file_extension=".yaml"
                ))

            # StressChaos for critical only
            if criticality == "critical":
                stress_content = (self.stress_chaos_template
                                 .replace("{{name}}", base_name)
                                 .replace("{{project_id}}", project_id)
                                 .replace("{{namespace}}", namespace)
                                 .replace("{{app}}", app_name))
                ...
                chaos_tests.append(GeneratedTest(
                    name=f"{base_name}_stress_chaos",
                    test_type=TestType.CHAOS,
                    derived_from=f"availability={requirements.get('availability', '99.9%')}, criticality={criticality}",
...
def write_tests(tests: List[GeneratedTest], output_dir: Path) -> List[Path]:
    ...
    Args:
        tests: List of generated tests to write
